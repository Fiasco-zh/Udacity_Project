...
class LearningAgent(Agent):
    ...
    def reset(self, destination=None, testing=False):
        ...
        if testing == True:
            ...
            self.trail += 1
            self.epsilon = math.exp(-self.trail/700.)
        return None
    ...
    def choose_action(self, state):
        ...
        best_action = {k: v for k,v in self.Q[state].iteritems() if v==self.get_maxQ(state)}.keys()
        # best_action = [action for action,v in self.Q[state] if v == self.get_maxQ(state)]
        # new_action = {k: v for k,v in self.Q[state].iteritems() if v==0}.keys()
        ...
        if not self.learning:
            ...
            # if new_action:
            #     action = random.choice(new_action)
            # else:
